...
Thank you for considering contributing to FindMyCrib_Africa! We welcome all kinds of contributions, including bug reports, feature requests, and code improvements.
...
1. Clone your forked repository:
   ```sh
   git clone https://github.com/FindMyCrib-Africa/FindMyCrib_frontend.git
   ```
...
1. Create a new branch for your work:
   ```sh
   git checkout -b feat/FMC-2145-your-feature-name
   ```
   ##### Branch Naming Rules
   ...
   - For any of these updates, you will likely use a ticket or an issue. The ticket number, e.g. FMC-2145 or issue number should also be included in your branch name
   - Finally, a short description for your update should follow suit. This is often taken from the ticket title
   - All of this (except the ticket number acronym, `FMC`) should be written in lowercase
     > Thus, a typical branch should look like `feat/FMC-1234-create-login-page` or like `chore/remove-unused-variables` if your update has no corresponding ticket or issue (unlikely)
2. Make your changes, and commit them with descriptive messages:
   ...
   Commit messages also follow a similar pattern. However, there is no need to add ticket number since they can be easily tracked given the branch name. Instead, use a colon, `:`, after the type of change (`feat`, `fix`, etc.), a whitespace, then your commit message. In cases where you are required to add the ticket number, you may use a the parenthesis after the type of change, like `feat(FMC-1234): your commit message`

   > Another example: `refactor: use a single state for formData` or `refactor(FMC-1234): use a single state for formData`
...
1. Ensure your branch is up to date with the cloned repository:
